...
# 1. Xác định cổng máy chủ đang chạy
$port = 8080
...
# Đảm bảo máy chủ nội bộ đang chạy
$serverRunning = $false
try {
    $resp = Invoke-WebRequest -Uri "http://localhost:$port/api/version" -TimeoutSec 1 -UseBasicParsing -ErrorAction Stop
...
# 2. Khởi chạy Cloudflare Tunnel với đúng cổng máy chủ
Write-Host "Đang kết nối mạng toàn cầu Cloudflare tới máy chủ (Port: $port)..." -ForegroundColor Yellow
